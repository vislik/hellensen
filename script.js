function getSavedLang() {
  try {
    const value = localStorage.getItem("olga-site-lang");
    return value || "ru";
  } catch (error) {
    return "ru";
  }
...
const translations = {
  ru: {
    pageTitle: "Ольга Назарова | Стоматолог в Нижнем Новгороде",
    pageDescription:
    ...
    navNews: "Новости",
    heroKicker: "Частный стоматолог в Нижнем Новгороде",
    ...
    bioPortraitLabel: "Личный прием",
    bioPortraitText: "Работаю аккуратно, объясняю каждый этап и сопровождаю пациента до результата.",
  ...
  en: {
    pageTitle: "Olga Nazarova | Dentist in Nizhny Novgorod",
    pageDescription:
    ...
    navNews: "News",
    heroKicker: "Private Dentist in Nizhny Novgorod",
    ...
    bioPortraitLabel: "Private Appointment",
    bioPortraitText: "Careful treatment, clear explanations, and support from first visit to final result.",
...
function setLanguage(nextLang) {
  ...
  try {
    ...
    // Ignore storage restrictions in embedded/file contexts.
  }
...
function setupImageFallbacks() {
  document.querySelectorAll("img[data-fallbacks]").forEach((image) => {
    const fromData = image.dataset.fallbacks
      .split("|")
      .map((item) => item.trim())
      .filter(Boolean);
    ...
    const localVariants = [];
    if (src.includes("olga-1.jpg")) {
      localVariants.push(
        "assets/olga-1.jpg",
        "/assets/olga-1.jpg",
        "file:///Users/macbook/Documents/New%20project/assets/olga-1.jpg",
      );
    }
    if (src.includes("olga-2.jpg")) {
      localVariants.push(
        "assets/olga-2.jpg",
        "/assets/olga-2.jpg",
        "file:///Users/macbook/Documents/New%20project/assets/olga-2.jpg",
      );
    }

    const queue = [...new Set([...localVariants, ...fromData])].filter((item) => item !== src);
...
function init() {
  try {
    ...
    // Keep the page usable even if some browser feature is unavailable.
